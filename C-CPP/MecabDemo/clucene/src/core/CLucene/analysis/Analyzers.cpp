...
SKBTokenizer::SKBTokenizer(CL_NS(util)::Reader* in):CharTokenizer(in) {
}
SKBTokenizer::~SKBTokenizer(){
}

bool SKBTokenizer::isTokenChar(const TCHAR c)  const{
    if( c >= 'a' && c <= 'z' )
        return true;
...
TCHAR SKBTokenizer::normalize(const TCHAR chr) const {
    return _totlower(chr);
}
...
SKBAnalyzer::SKBAnalyzer(){
}

SKBAnalyzer::~SKBAnalyzer(){
}

TokenStream* SKBAnalyzer::tokenStream(const TCHAR* /*fieldName*/, Reader* reader) {
    //return _CLNEW SKBFilter(_CLNEW SKBTokenizer(reader),true);
    return _CLNEW SKBTokenizer(reader);
}
TokenStream* SKBAnalyzer::reusableTokenStream(const TCHAR* fieldName, CL_NS(util)::Reader* reader)
{
    //return tokenStream(fieldName, reader);
    Tokenizer* tokenizer = static_cast<Tokenizer*>(getPreviousTokenStream());
    if (tokenizer == NULL) {
        tokenizer = _CLNEW SKBTokenizer(reader);
        setPreviousTokenStream(tokenizer);
    } else
        tokenizer->reset(reader);
    return tokenizer;
...
SKBFilter::SKBFilter(TokenStream* in, bool deleteTokenStream):
TokenFilter(in,deleteTokenStream) {
}
SKBFilter::~SKBFilter(){
}

Token* SKBFilter::next(Token* t){
