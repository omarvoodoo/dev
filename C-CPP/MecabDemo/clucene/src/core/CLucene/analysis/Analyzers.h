...
// SKB용 Tokenizer
class CLUCENE_EXPORT SKBTokenizer: public CharTokenizer {
public:
    /** Construct a new SKBTokenizer. */ 
    SKBTokenizer(CL_NS(util)::Reader* in);
    virtual ~SKBTokenizer();
protected:
    /** 영문자에 숫자또는 특수문자(- 또는 _)를 포함한 문자열을 수집한다. */
    bool isTokenChar(const TCHAR c) const;
...
class CLUCENE_EXPORT SKBAnalyzer: public Analyzer {
public:
    SKBAnalyzer();
    TokenStream* tokenStream(const TCHAR* fieldName, CL_NS(util)::Reader* reader);
    TokenStream* reusableTokenStream(const TCHAR* fieldName, CL_NS(util)::Reader* reader);
...
class CLUCENE_EXPORT SKBFilter: public TokenFilter {
public:
    SKBFilter(TokenStream* in, bool deleteTokenStream);
    virtual ~SKBFilter();
    Token* next(Token* token);
};
